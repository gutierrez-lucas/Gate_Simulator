...
int main(void)
{
	...
	printf("\r\n------------ Gate Simulator ------------\r\n");
	gs_s* gate = gate_simulator_init(GS_CLOSED);
	
	uint32_t time_reference = HAL_GetTick();
	while (1){
		if(HAL_GetTick() - time_reference > 20){
			time_reference = HAL_GetTick();
			gate_simulator_run(gate);
		}
	}
	gate_simulator_end(gate);
